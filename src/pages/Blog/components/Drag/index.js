...
import Taro, { vibrateShort, pageScrollTo, getSystemInfoSync } from '@tarojs/taro'
...
function Drag ({
  ...
  const initDom = (_listData) => {
    ...
    Taro.createSelectorQuery().select('.drag-item').boundingClientRect((res) => {
      const newRows = Math.ceil(_listData.length / columns)
      setRows(newRows)
      setItemDom(res)
      setItemWrapHeight(newRows * res.height)
      // eslint-disable-next-line no-undef
      Taro.createSelectorQuery().select('.drag-wrapper').boundingClientRect(rese => {
        const domObj = rese
  ...
  const init = () => {
    ...
    setTimeout(() => initDom(_listData), 50)
  }
    ...
    >
      {
        listData.map((v, index) => (
          <View
            ...
            onLongPress={moveable && longPress}
            catchMove
            onTouchMove={moveable && draging && touchMove}
            onTouchEnd={moveable && draging && touchEnd}
          >

            <View className={`drag-item__info ${cur === index ? 'drag-active' : ''}`}>
              <View className='drag-item__info-item'>
                {
                  typeof renderItem === 'function' ? renderItem(v, index) : 
                  !v.extraNode ? <>
					          {/* 这里逻辑可以自己写 */}
