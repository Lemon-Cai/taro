...
export default class FilterDropdown extends Component {
  ...
  showMoreSub = index => {
    ...
    subData[this.state.showPage].submenu[this.state.activeMenuArr[this.state.showPage][0]].submenu[
      index
    ].showAllSub = true
    this.setState(
      {
    ...
    this.setState(
      ...
      () => {
        ...
        if (
          ...
          level2_index != null ||
          (level1_index != null &&
            this.state.subData[page_index].submenu[level1_index].submenu.length == 0)
        ) {
  ...
  selectRadioLabel = (page_index, box_index, label_index) => {
    ...
    if (activeIndex == label_index) {
      ...
      if (
        activeIndex != null &&
        activeIndex < this.state.subData[page_index].submenu[box_index].submenu.length
      ) {
        subData[page_index].submenu[box_index].submenu[activeIndex].selected = false
      }
      subData[page_index].submenu[box_index].submenu[label_index].selected = true
      ...
      if (typeof item[0] == 'object') {
        ...
        item.forEach((s, j) => {
          if (s != null) {
            ...
            s.forEach((v, k) => {
              value[i][j][k] =
                v == null || v >= this.state.subData[i].submenu[j].submenu.length
                  ? null
    ...
    return (
      <View className='index'>
        <View className='HMfilterDropdown' catchtouchmove={true}>
                ...
                <Fragment key={index}>
                  <View
                    ...
                    <Text
                      ...
                      style={'transform:rotate(' + triangleDeg[index] + 'deg);'}></Text>
                  </View>
                </Fragment>
              )
            })}
          ...
          <View
            className={
              'mask' + (isShowMask ? ' show' : '') + (maskVisibility != true ? ' hide' : '')
            }
            onClick={this.togglePage.bind(this, showPage)}></View>
                  ...
                  showPage === page_index ? (
                    <Fragment>
                      <ScrollView
                        className={
                          'sub-menu-list' +
                          (activeMenuArr[page_index].length > 1 ? ' first' : ' alone')
                        }
                        scrollY={true}
                        ...
                        {page.submenu.map((sub, index) => {
                          return (
                            <Fragment key={index}>
                              <View
                                className={
                                  'sub-menu' + (activeMenuArr[page_index][0] == index ? ' on' : '')
                                }
                                id={'first_id' + index}
                    ...
                    <Fragment>
                      <View className='filter'>
                        ...
                        <View className='btn-box'>
                          <View
                            className='reset'
                            onClick={this.resetFilterData.bind(this, page_index)}>
                            重置
                          </View>
                          <View
                            className='submit'
