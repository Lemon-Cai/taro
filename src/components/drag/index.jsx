    ...
    <View className='drag-wrapper' style={`height: ${itemWrapHeight}px`}>
      {listData.map((v, index) => (
        <View
          ...
          data-key={v.sortKey}
          data-index={index}
          style={`transform: translate3d(${index === cur ? tranX + 'px' : v.tranX}, ${
            index === cur ? tranY + 'px' : v.tranY
          }, 0);width: ${itemWidth ||
            'calc((100% - ' + 8 * columns + 'px) / ' + columns + ')'}; z-index: ${
            index === cur ? curZ : ''
          }`}
            ...
            <View className='drag-item__info-item'>
              {typeof renderItem === 'function' ? (
                ...
                <>
                  ...
                  {!draging && (
                    <View className='drag-item__info-close' onClick={() => delEvent(index)}>
                      X
                    </View>
                  )}
                </>
